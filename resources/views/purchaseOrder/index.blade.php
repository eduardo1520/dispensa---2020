                ...
                <div class="row " align="center" id="tabela">
                    <div class="col-8 col-lg-12 my-3 border " id="filho">
                        ...
                        @forelse($purchase_orders as $data => $listas)
                            ...
                            @endphp
                            @php
                                $nome = current(current($listas))['nome_completo'] ?? '';
                                $sexo = current(current($listas))['sexo'] ?? '';
                                $status = current(current($listas))['status'];
                                $id = current(current($listas))['id'];
                            @endphp
                            <style>
                                .pedido_{{$status == 'C' ? $id : "$arr[0]_$arr[1]"}} tbody tr:nth-of-type(odd) {
                                    @if($arr[2] == 'Aguardando aprovação')
                                         background-color: #f6c23e;
                                    @elseif($arr[2] == 'Cancelado')
                                         background-color: #FF6347;
                                    @elseif($arr[2] == 'Aprovado')
                                         background-color: #90EE90;
                                    @endif
                                    cursor:pointer;
                                }
                            </style>
                            <table class="mt-lg-3 table table-striped table-bordered  table-responsive-lg pedido_{{$status == 'C' ? $id : "$arr[0]_$arr[1]"}}" style="border-collapse:collapse;">
                                <tbody>
                                <tr colspan="6" data-toggle="collapse" data-target=".demo_{{$status == 'C' ? $id : "$arr[0]_$arr[1]"}}" class="accordion-toggle">
                                    @php
                                        $card = '';
                                        $classe = '';
                                        if($arr[2] == 'Cancelado') {
                                        ...
                                        if(!empty($qtdes[$arr[0]][$arr[1]])) {
                                            $pedidos = $qtdes[$arr[0]][$arr[1]];
                                        }
                                    @endphp
                                    <td>
                                        <div class="row">
                                            <div class="col-md-1">
                                                ...
                                                <span class="btn btn-{{ $classe }} btn-circle btn-sm" style="margin-left: -40;padding-left: 0px;padding-top: 0px;padding-right: 0px;border-right-width: 0px;margin-right: 0px;margin-top: -45;padding-bottom: 0px;border-bottom-width: 4px;height: 25px;width: 28px;">
                                                    {{!empty($pedidos) ? count($pedidos) : '1'}}
                                                </span>
                                            </div>
                                            <div class="col-md-5 mt-2">
                                                @if(!empty($arr[1]))
                                                    Lista de Pedidos - Data: {{ $arr[2] == 'Cancelado' || $arr[2] == 'Aprovado' ? date('d/m/Y H:i:s',$arr[1]) : date('d/m/Y',$arr[1]) }} - ({{ $arr[2] }})
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div style="float: right; margin-right: 5px; margin-bottom: 5px; {{ $arr[1] == 'Cancelado' ||  $arr[1] == 'Aprovado' ? 'display:none' : 'cursor:wait;'}}">
                                            @if(!empty(\Auth::user()->admin) &&  \Auth::user()->admin == 'S')
                                                <a href="#" class="btn btn-info btn-circle btn-sm user" title="Visualizar Usuário" onclick="buscarUsuario('{{$nome}}','{{$sexo}}')"><i class="fas fa-user"></i></a>
                                                @if($status == 'P')
                                                    <a href="#" class="btn btn-success btn-circle btn-sm aprovar" title="Aprovar Pedido" onclick="aprovarPedidoLista({{ $arr[1] }},'111111',{{$arr[0]}})"><i class="fa fa-thumbs-up"></i></a>
                                                    <a href="#" class="btn btn-danger btn-circle btn-sm excluir" title="Excluír Produto" onclick="removePedidoLista({{ $arr[1] }},'999999','')"><i class="fas fa-trash"></i></a>
                                                @endif
                                            @endif
                                        </div>
                                    </td>
                                                                ...
                                                                <div class="card mb-3" style="max-width: 400px;">
                                                                    <div class="row g-0">
                                                                        ...
                                                                        <div class="col-md-8">
                                                                            ...
                                                                            @php
                                                                                if($p['status'] == 'P' && $arr[1] >= strtotime(date('Y-m-d', strtotime('-5 day')))):
                                                                                    $disabled = "";
                                                                                elseif($p['status'] == 'A' || $p['status'] == 'C' || $arr[1] <= strtotime(date('Y-m-d', strtotime('-5 day')))):
                                                                                    $disabled = "display:none";
                                                                                endif;
                                                                            @endphp
                                                                            <div style="float: right; margin-right: 5px; margin-bottom: 5px;{{$disabled ?? ''}}" >
                                                                                <a href="#" class="btn btn-danger btn-circle btn-sm excluir" title="Excluír Produto" data-id="{{ $p['id'] }}" onclick="removePedidoLista({{ $arr[1] }},{{ $p['id'] }}, '{{ $p['product_name'] }}')"><i class="fas fa-trash"></i></a>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
